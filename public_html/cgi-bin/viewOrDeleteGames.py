...
query = "SELECT game.game_id, a.team_name, b.team_name, game.game_date FROM game JOIN team a ON game.home_team_id = a.team_id JOIN team b ON game.away_team_id = b.team_id" 
...
for (game_id, home_team, away_team, date) in cursor:
    print('<tr>')
    print('<td>' + str(date) + '</td>')
    print('<td>' + home_team + '</td>')
    print('<td>' + away_team + '</td>')
